...
void FrameBufferEx::AttachColorBuffer(VkFormat inFormat /* = VK_FORMAT_R8G8B8A8_UNORM */) {
	// Offscreen color attachment that can later be sampled in shaders.
...
void FrameBufferEx::AttachDepthBuffer() {
	// Depth attachment is also sample-capable for potential depth-based effects.
...
void FrameBufferEx::Finish(VkImageLayout inColorBufferFinalLayout) {
	// Build a dedicated render pass and framebuffer for current attachment list.
...
VkCommandBuffer FrameBufferEx::BeginRendering(VkCommandBuffer commandbuffer /* = nullptr */) {
	// Begin render pass on this offscreen FBO.
	VkCommandBuffer cmd;
