...
Material::Material(const char* inVSPath, const char* inFSPath) {
	// Build shader program from precompiled SPIR-V binaries.
	...
	if (mDefaultSampler == nullptr) {
		// Shared fallback sampler used when caller does not provide one.
...
void Material::Active(VkCommandBuffer inCommandBuffer, int inVertexDataSize) {
	// Ensure vec4 uniform block (binding 3) exists and is up-to-date.
	...
	if (mbNeedUpdatePSO) {
		...
		// Pipeline is created lazily because vertex layout may differ per mesh.
	...
	// Descriptor writes are cached as templates; patch dstSet each frame.
