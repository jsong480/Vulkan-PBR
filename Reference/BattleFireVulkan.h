...
// Framebuffer wrapper for system swapchain frame resources.
struct FrameBuffer {
...
// Two-stage graphics shader program (VS + FS).
struct GPUProgram {
...
// Generic Vulkan buffer + allocation pair.
struct BufferObject {
	VkBuffer mBuffer;
	VkDeviceMemory mMemory;
	int mSize;
...
// CPU-side matrix block mirrored to a single UBO.
...
// CPU-side vec4 block mirrored to a single UBO.
...
// Global descriptor set layout definition used by all materials.
...
// Pipeline state container (render pass + viewport + pipeline object).
